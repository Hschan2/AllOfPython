...
training_data_file = csv.reader(codecs.iterdecode(response, 'UTF-8')) # UTF-8로 읽기
training_data_list = list(training_data_file)
all_values = training_data_list[0]
image_array = np.asfarray(all_values[1:]).reshape((28,28))
plt.imshow(image_array, cmap = 'Greys', interpolation = 'None')
plt.savefig('textRecognition.png') # textRecognition.png로 저장
response.close()
...
class neuralNetwork:
    ...
    def train(self, inputs_list, targets_list):
        ...
        self.who += self.lr * np.dot((output_errors * final_outputs * (1.0 - final_outputs)), numpy.transpose(hidden_outputs))
        # 입력 계층과 은닉 계층 간의 가중치 업데이트
        self.wih += self.lr * np.dot((hidden_errors * hidden_outputs * (1.0 - hidden_outputs)), numpy.transpose(inputs))
...
## Tesseract(pytesseract)를 이용하여 이미지 글자(숫자) 인식
...
img = Image.open('글자or숫자가 있는 파일.png')
text = pytesseract.image_to_string(img, lang = 'kor + eng') # 이미지에서 영어 혹은 한글 가져오기 (숫자도 포함), kor + eng로 했을 때 한글 인식률 떨어짐. kor만 했을 시 한글 인식률 증가
